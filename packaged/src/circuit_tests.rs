...
mod tests {
    ...
    use crate::{
        data::{Bit, Bit::One, Bit::Zero, Circuit, Shift, Mul, Adder},
        dividers::{DivInfo, Estimate, IntDivResult, SubMethod, Method},
    };
    use rand::random;
    ...
    fn test_adder_value(adder: fn(&mut Circuit, Vec<Bit>, Vec<Bit>, Bit) -> Vec<Bit>, value1: usize, value2: usize) {
        let mut circuit = Circuit::new();
        let (s1, v1) = (value1, Bit::get_bits_vec_usize(value1));
        let (s2, v2) = (value2, Bit::get_bits_vec_usize(value2));

        let (r, _) = s1.overflowing_add(s2 + 1);

        let r_calc = adder(&mut circuit, v1.clone(), v2.clone(), One);
        let r_calc = match Bit::get_number_u(&r_calc) {
            Ok(r) => r,
            Err(_) => { return; }
        };
    ...
    fn adders_random_size() {
        ...
        test_adder_value(Circuit::ksa, 0b0, 0b1);
    }
    ...
    fn test_mul_u_dadda_fixed_size() {
        ...
        for i in 0..TEST_SIZE_BIG {
            ...
            let p_calc = circuit.umul_dadda(v1, v2, Some(v3), crate::data::Adder::KSA);
    ...
    #[test]
    fn test_lt_simple_fixed_size() {
        let mut circuit = Circuit::new();
        for i in 0..=255 {
            for j in 0..=255 {
                let mut a = Bit::get_bits_vec_u8(i);
                let mut b = Bit::get_bits_vec_u8(j);
                
                let lt = i < j;
                let eq = i == j;
                a.truncate(8);
                b.truncate(8);
                println!("a as vec: {:?}, b as vec {:?}", { a.reverse(); &a}, { b.reverse(); &b});
                a.reverse();
                b.reverse();
                
                println!("a: {i}, b: {j}");

                let (lt_calc, eq_calc) = circuit.less_than(&a, &b);
                // println!("{:?} {:?}", lt_calc, eq_calc);
                let lt_calc = Bit::get_number_u(&vec![lt_calc]).unwrap();
                let eq_calc = Bit::get_number_u(&vec![eq_calc]).unwrap();

                assert_eq!(lt_calc, lt as usize);
                assert_eq!(eq_calc, eq as usize);

                if !(lt_calc == lt as usize && eq_calc == eq as usize) {
                    println!("some_thing_to_correct! a: {}, b: {}", i, j);
                }
            }
        }
    }
    ...
    #[test]
    fn test_div_simple_fixed_size_restoring() {
        let mut circuit = Circuit::new();
        for d_num in 0..=255 {
            for z_num in 1..=255 {
                let mut d = Bit::get_bits_vec_u8(d_num);
                ...
                info.defaultmult = Mul::DadaTree;
                let result = circuit.restoring_division(d, z, info);
                let q_calc = Bit::get_number_u(&result.q).unwrap();
                // println!("q calculated: {q_calc}");
                let r_calc = Bit::get_number_u(&result.r).unwrap();
                // println!("r calculated: {r_calc}");
    ...
    #[test]
    fn test_div_restoring_example() {
        let mut circuit = Circuit::new();
                let mut d = vec![One, One, One, One, One, One, One, One, One, One, One, One];
                let mut z = vec![One, One, One, One, One, One, One, One, One, One, One, Zero];
    ...
    #[test]
    fn test_div_simple_fixed_size_with_poly_spec() {
        let mut circuit = Circuit::new();
        for d_num in 0..=255 {
            for z_num in 0..=255 {
                let mut d = Bit::get_bits_vec_u8(d_num);
                ...
                let info = DivInfo::default_newton();
                let result = circuit.div_newton(d.clone(), z.clone(), info);
                let result = circuit.umul_dadda(z, result.q, Some(result.r), crate::data::Adder::CRA);
                let result = circuit.crs(d, result, Zero);
                let calc = Bit::get_number_u(&result).unwrap();
                // println!("{calc}");
                assert_eq!(0, calc);
            }
        }
    ...
    fn test_poison() {
        let mut circuit = Circuit::new();
        for _ in 0..TEST_SIZE_SMALL {
            let (_, s1) = get_random_number_with_fixed_digits(16);
            let (_, s2) = get_random_number_with_fixed_digits(16);

            let sum1 = circuit.cra(s1.clone(), s2.clone(), Zero);
            let sum2 = circuit.ksa(s1.clone(), s2.clone(), Zero);
            let zero = circuit.crs(sum1, sum2, Zero);

            println!("{}", Bit::get_number_u(&zero).unwrap());
        }
    }
}
