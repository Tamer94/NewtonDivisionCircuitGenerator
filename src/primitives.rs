...
impl LevelizedCircuit {
    // these implementations cant be used because we need constant propagation as for now because of the polynom engine
    ...
    /*
    #[inline(always)]
    pub fn levelize_gate(&mut self, gate: Gate) -> Line {
        self.circuit.stats.gatter_count += 1;
        let line = Line {
            level: gate.get_next_level(),
            n: self.circuit.stats.add_line(),
    ...
    pub fn xor(&mut self, i1: Bit, i2: Bit) -> Bit {
        ...
        match (i1, i2) {
            (Var(j1), Var(j2)) => {
                if j1 != j2 {
                    let gate = Gate::Xor(j1, j2);
                    let line = self.levelize_gate(gate);
                    bit = Var(line);
